...
- **The closed loop — self-learning as a checkable property (SPEC §23, BUGS.md BUG-23).** Storing, verifying, and serving intelligence isn't self-learning until the system OBSERVES its own learning and directs it. Five mechanisms: (1) **retrieval-miss telemetry** — every graph search that finds nothing logs to `.atlas/retrieval-misses.jsonl` (`atlas skill find` already writes it); misses ARE the curriculum, the literal list of intelligence the fleet needed and didn't have; (2) **graph-health KPIs** — `atlas measure --knowledge [--json]`: skills by status, certification coverage, eval runs/failures, retrieval misses, staleness debt — the system watches its own learning rate and the loop targets whatever degrades; (3) **the knowledge-loop cron** — nightly cheap-model run: mine transcripts for discovery arcs → draft skills (capture is a pipeline, not a virtue — production evidence: SCARS got 12 commits in 36 h while the skill registry got 0) → re-run evals + decay certifications → apply update triggers → ingest misses → emit the KPI delta; (4) **the critic pass** — adversarial QC (executable by a weak model? eval discriminates? duplicate? poisoning risk?) before any draft activates, itself a certified skill; (5) **meta-skills** — the loop's own procedures are skills in the graph held to the same 1B bar, so once certified for a small model the system that improves the fleet is maintained by the fleet, and frontier models are spent only on novel discovery. Ends with the completeness test: disconnect the frontier models for a week — does the fleet still capture, stay true, and measurably improve?
- **`SKILL_GRAPH.md` — distilled intelligence with a measured capability floor (SPEC §22, BUGS.md BUG-22, `templates/SKILL_GRAPH.md.tmpl`, generalized from CRMAIX).** Where a project's intelligence is stored — not model weights, not chat history: procedures written to the **1B-model test** (every step a concrete command, every branch an explicit IF/THEN, every step with its VERIFY, failure modes linked to `SCARS §ANCHORS`), so discovery is paid once by whoever is smartest that day and execution is cheap forever, any runtime, offline included. Registry table + honest "shipped but not yet distilled" section + typed mermaid graph. **Certification is measured, never asserted:** `atlas skill test <name> --tier <T>` runs the skill's executable eval and appends the record to `skills/certs.jsonl` — the `Certified` column may claim only tiers a passing record backs (a failing eval flips `status: broken`, loudly). **Symptom-indexed retrieval:** `atlas skill find --symptom "<paste error>"` greps the graph BEFORE debugging — the highest-leverage retrieval path; a miss is logged to `.atlas/retrieval-misses.jsonl` as the knowledge loop's demand signal (§23). `atlas init --skill-graph` scaffolds (`_render` learned `{{PROJECT_SLUG}}`); `atlas check` validates only-when-present: `SKILL_GRAPH_DANGLING` (registry row → missing file), `SKILL_NO_EVAL` (active skill without an eval), `SKILL_CERT_UNBACKED` (a claim with no passing record); hook surfaces the two retrieval rules. The hardening ladder (skill → script → runner via `compiled_to:`), federation as content-addressed export (markdown = source of truth, any DB a rebuildable index), the promotion security gate (review recorded in DECISION_GRAPH — a skill is executable instruction, poisoning is the fleet's largest attack surface), dead-end nodes, and skills-as-products are specified as §22.4/§22.5 conventions.
- **The supervisor role + evidence-bearing reports (SPEC §17.4/§18.3, BUGS.md BUG-19/BUG-20).** *Supervisor:* the role split that made 6 concurrent builders shippable with zero deploy collisions in ~35 deploys — builders build in slices and NEVER deploy; one supervisor deploys every commit, verifies live with evidence, owns rollbacks, routes inter-agent messages, and is the only role that talks to the owner; a permission-blocked builder routes the request TO the supervisor, never around them. *Reports:* every "verified" claim names its evidence (exact command, test count, screenshot path — a claim without evidence is a hypothesis), every report ends with a COST line (tokens/compute — mission ROI becomes measurable; handoff files already carry running cost), and boundaries ("what I did NOT change") are part of the report. Evidence is what the knowledge loop (§23) later distills; cost lines are what skill ROI (§22) computes from.
- **Update triggers per knowledge file + FAQ freshness gate (BUG-21, SPEC §6).** Each scaffolded file now states its own update duty in its own header, at read time, instead of relying on a separate policy doc: `ATLAS.md`/`SCARS.md`/`FAQ.md`/`DECISION_GRAPH.md` templates each gain a bold **Update trigger:** line, and `CLAUDE.md`'s commit-hygiene rules restate the FAQ trigger as a law ("a feature the FAQ cannot explain is not finished") so it survives even when an agent skips straight to coding. The FAQ trigger is also mechanically checkable: declare `<!-- atlas:user-facing: <glob> -->` once in `ATLAS.md`, and `atlas check --strict` warns `FAQ_STALE_VS_CODE` once 10+ commits have touched that glob since the FAQ last moved — the "36h dark-window" class, where user-visible behavior ships repeatedly while the ask-once ledger sits unread. No declaration means no check; a repo that hasn't opted in is fully unaffected.

- **Boot-context token budget (BUG-18, SPEC §6).** `ATLAS.md` and `CLAUDE.md` are read on every session; their cost is invisible until an agent gets measurably dumber, because nothing forces anyone to notice a file quietly growing. `atlas measure` now prints a "boot set" breakdown (`CLAUDE.md` / `ATLAS.md` / `SCARS.md`+`SKILL.md` Table-of-contents slices + a total, bytes÷4) before its existing with-vs-without summary. `atlas check --strict` warns `BOOT_BUDGET_EXCEEDED` when `ATLAS.md` exceeds ~2500 tokens or `CLAUDE.md` exceeds ~3000 tokens, with the fix being the §0 table pattern (move detail into a linked doc, keep the law as a one-liner citing it) — not shorter sentences. Note: this repo's own `ATLAS.md` is currently ~3137 est. tokens, over its own budget; per the module's own advice it needs a §0-table trim, not more prose, and is left as a flagged follow-up rather than trimmed in this slice.

- **Org-wide SCARS overlay + `atlas promote` (SPEC §21, BUG-17).** The same hard lesson (a `set -e && chain` trap, a `sed -i` portability gotcha) gets re-paid across sibling repos before anyone thinks to share it. `${ATLAS_GLOBAL_SCARS:-$HOME/.atlas/SCARS.md}` is one overlay file every repo's SessionStart hook reads and clearly labels as "ORG-WIDE SCARS (global overlay: `<path>`)" after the repo's own SCARS block — never silently merged in. `atlas promote <ANCHOR-NAME>` moves one anchor's full body to the overlay (with a "promoted from `<repo>`" Table-of-contents bullet), leaving a redirect stub in the repo file that keeps the anchor's id and ToC entry intact — anchors are immutable (§3.4), so old citations keep resolving. Refuses to re-promote an already-stubbed anchor. `atlas check --deep --strict` stays green on a repo after promotion (verified empirically: the deep anchor-schema checks exempt a body with no Symptom/Root-cause). The overlay is read-only context for agents — a scar is still authored in the repo first; promotion is a maintainer curation act.

- **`DECISION_GRAPH.md` — the judgment graph (SPEC §20, BUG-16/BUG-24).** SKILL.md stores *how*, SCARS.md stores *what breaks*, and now `DECISION_GRAPH.md` stores *why a call was made and what re-opening it would touch* — the third leg of the knowledge fabric, so agents stop re-litigating settled decisions. Nodes (`D-<slug>`, `pending|decided|superseded`, decider, statement, evidence, rejected options) join via typed edges (`supersedes` · `constrains` — the blast-radius edge · `motivated-by` a SCARS `§ANCHOR` · `licenses`/`forbids`/`blocks`), never deleted, only superseded. Two agent laws bind every runtime: never re-open a `decided` node; never act silently where a `pending` node exists — act on its default or escalate. `atlas init --decisions` scaffolds it (scaffold-missing-only); `atlas check` warns `DECISION_PENDING_STALE` on a Pending row older than 14 days — a rotting pending decision is a silent blocker; the SessionStart hook prints the two laws plus a pending-count line whenever the file exists. Note: BUG-16's "warn when the surface is entirely absent" was deliberately NOT implemented — every optional surface in ATLAS follows the only-when-present convention (see BUGS.md discussion); a repo that doesn't use decision graphs is fully unaffected, exactly like LOOP.md/BUGS.md/CRITICS.md/FAQ.md.
...
- **Operational doctrine modules (SPEC §18, BUGS.md BUG-11/BUG-12).** Campaign-proven conventions stated as spec modules any runtime reads identically. *§18.1 Long-job runners:* a job that outlives one agent session is staged+checkpointed (manifest, idempotent reruns), detached, heartbeating through an existing channel, loud on failure (the SIGPIPE class explicitly — a `find | head` under pipefail killed a runner mid-verify with status stuck "running"), watched by a dead-pid monitor that turns "pid gone while status=running" into an honest FAILED, and documents start/check/kill/resume. Every runner built this way survived agent deaths, spend-limit kills, an internet outage, and an egress switch; every job that wasn't died silently or double-ran. *§18.2 Shipping discipline:* slice-commit (first green slice lands immediately), deploy-per-commit by the supervisor, verification-as-evidence (a live screenshot, not a claim), and safe concurrent builds from a worktree of HEAD.
- **Data-source ladder — the licence law as an optional module (SPEC §19, BUGS.md BUG-14, `templates/DATA_SOURCE_LADDER.md.tmpl`).** For data-product repos: the four-rung law (free/open first → commercial/NC alternatives CATALOGued as activatable paid upgrades, never silently ingested → unclear licences get a tracked NEGOTIATION state → NC-blocked datasets REBUILT from open inputs as branded first-party datasets), the machine-readable catalog shape, the 8-step Dataset Minting Checklist (each step carrying its verification), and the truth test ("could a paying customer, an auditor, and the data provider all read our catalog entry and find it exactly true?"). Licence compliance becomes a product surface — the catalog IS the upsell page. `atlas init --data-ladder` renders `docs/DATA_SOURCE_LADDER.md` and appends the abbreviated law to `CLAUDE.md` (re-mirroring `AGENTS.md` only when it was in sync — a pre-existing drift keeps failing loudly); `atlas check` warns `DATA_LADDER_NO_LAW` when the ladder file exists but the contract doesn't link it. Generalized from OrbiVigil's production implementation.
...
- **`FAQ.md` — the Q&A knowledge ledger, an OPTIONAL surface (SPEC §13, user-directed 2026-07-25).** The ask-once file: questions a human asked and the answers/surprising findings an agent surfaced — *"why does it do that?"*, *"why not X?"* — answered once, with pointers, so the next session looks it up instead of re-deriving it at full cost. Generalized from proxima-finance's production `docs/FAQ.md`. Scalable by convention: stable `FAQ-NNN` keys (permanent, citable, knowledge-graph-extractable) and append-only entries (corrections are dated `**Update:**` appends), which keeps concurrent agents merge-safe. Lives at root `FAQ.md` **or** `docs/FAQ.md` — tooling treats both identically, so proxima's reference file conforms unmodified. `atlas init --faq` scaffolds `templates/FAQ.md.tmpl` at `docs/FAQ.md` (the location the default `ATLAS.md` §1.2 row already anticipates), auto-linked from `ATLAS.md`. `atlas check` validates **only when present**: `FAQ_MD_UNLINKED` warns when a public FAQ isn't linked from `ATLAS.md` **at its actual location** — a plain-text mention is not a link (SCARS §BUGS-LINK-NOT-SUBSTRING), and the template's leftover `docs/FAQ.md` row does not cover a root `FAQ.md` (a dead link is not orientation); git-ignored FAQs are a valid private choice and are skipped. Reported as `"faq"` in `--json`; listed in the `llms.txt` export read-first set (+ the `atlas fix` regeneration trigger, the BUG-7 consistency rule); SessionStart hook pointer. Graduation convention: failure-lesson → SCARS `§ANCHOR`, procedure → SKILL recipe, open unknown → BUGS.md. This repo dogfoods it: `docs/FAQ.md` seeded with five real answered questions.
- **Tool context contract — `.atlas/tools.json` + `atlas measure --tools` (SPEC §14, BUGS.md FEAT-4 core).** ATLAS told an agent what to read but nothing about how external graph/index/context tools identify the repo, prove freshness, or keep session plumbing out of subprocesses — ProximAgent dogfooding hit all three (a tool inventing a path-derived project id; a vanished `NODE_OPTIONS` preload failing child tests; silently-stale graph results). Now: **`atlas init --tools`** writes a generated, LOCAL, git-ignored `.atlas/tools.json` (one key per line by contract, so zero-dep consumers parse it without jq): canonical `project_id` (the §1 kebab slug every tool MUST accept instead of inventing one), repo root, `head`/`dirty` at generation, and `indexed_head`/`indexed_at` — `null` until an indexer stamps what it actually indexed (regeneration carries the stamp over; the `.gitignore` entries are added only when a broader pattern doesn't already cover them). **`atlas check`** validates only-when-present: `TOOLS_JSON_INVALID` (no `project_id`), always reports `"tools": {present, project_id, indexed_head, head, fresh, dirty}` in `--json`, and `--deep` warns `TOOLS_INDEX_STALE` when `indexed_head` ≠ HEAD — warn, never silently trust. **`atlas measure --tools [--json]`** is the one-command report: identity, index freshness, worktree drift, the §14.3 env contract (flags dead `NODE_OPTIONS --require` preloads), and aggregates of the spec'd `.atlas/tools-telemetry.jsonl` ledger tools append to (events · bytes indexed→returned · retries · stale hits · failures · estimated context tokens kept out of the conversation). FEAT-4's remaining seams (evidence separation, signed job broker, graph-RAG provenance, capability probe, release-evidence manifest) are defined normatively in SPEC §14.4, explicitly not yet CLI-enforced. A repo without the file is fully unaffected.

- **`atlas bench --matrix "model-a,model-b"` — free cross-model benchmark matrix (RM-4, SPEC §8).** Matrix mode repeats the existing with/without ATLAS benchmark for each listed runtime model, keeps the normal summary ledger as one comparison row per model, and writes `results/matrix-ledger.jsonl` with one raw row per model × condition × repetition. This makes local opencode/model sweeps auditable without changing the released single-model report format.
- **`atlas loop log [--json]` — loop observability from the iteration ledger (RM-20, SPEC §8).** The autonomous-loop report is overwritten every iteration, while the useful audit trail lives in either a local `LOOP_HISTORY.md` append ledger (for repos that git-ignore live loop state) or tracked `LOOP_REPORT.md` history. The new command reads both sources, extracts each iteration's provenance header (timestamp · model · effort · duration · sub-agents), ticket, and impact-check, and renders a newest-first Markdown table; `--json` emits the same fields for dashboards.
- **ASOP — operating methodology, a canonical companion surface (SPEC §12).** ATLAS covers *orientation* (ATLAS.md) and *memory* (SCARS.md) well, but was thin on the two axes where a weaker agent's output diverges from a stronger one's: *verification* (proving a change works, not declaring done on a proxy) and *escalation* (handing up when a task is above tier, not proceeding confidently wrong). Two new root-level canonical docs close that gap: **`ASOP.md`** (13 reasoning moves + a five-question pre-send self-test) and **`ASOP-EXECUTOR.md`** (a one-page, role-based mechanical card: pre-flight → verify → **escalate** → report). Both are OPTIONAL and **copied verbatim, not customized per-repo** — so there is *no new required conformance check* and the required set is unchanged. This repo dogfoods it: `ATLAS.md` §0 now surfaces the repo's *done-gate* and *escalation line* in one hop. **`atlas init --asop` scaffolds both files verbatim** (`--force` refreshes them, since they're canonical), and `atlas init` now seeds the done-gate + escalation-line rows into the default `ATLAS.md` §0 template. `atlas check --deep` warns `ASOP_STALE` when a present ASOP file drifts from the shipped canonical version (advisory — canonical files are refreshed, not diffed as custom).
...
- **`CRITIC_DISPATCH_BROKEN` — consecutive-failed-dispatch detection for `CRITICS.md` (`atlas check --deep`, idea-ledger item).** `atlas critique` already records a failed dispatch distinctly (RM-43), but nothing surfaced a STREAK — a repo whose critic integration broke (expired auth, a CLI flag change) would silently accumulate failed entries forever, invisible to `CRITICS_STALE` (which only fires on zero attempts). Now warns when the 3 most recent `### <date> — <topic>` entries are all a `**DISPATCH FAILED**` marker. Fence-aware by construction (mirrors the RM-45 `CRITICS_STALE` fix): a successful entry whose captured raw output happens to contain that literal string inside its fenced block does not count as a failure. `--deep`-only; same git-ignored exemption as `CRITICS_STALE`.
- **`atlas remember` — the write side of ATLAS as a read-write memory layer, core (RM-21, SPEC §16).** Everything an agent learns mid-session previously died at compaction/session end unless a human curated it. `atlas remember "<note>" --kind scar|skill|bug|faq [--name NAME]` distills a durable learning back into the repo instead. `--kind` is **REQUIRED** (scope-reduced from the ROADMAP ticket, stated explicitly): no keyword auto-classifier — a fake classifier would be dishonest about what it's actually confident of. `scar` appends a stub `§ANCHOR` to `SCARS.md` **plus its ToC entry** — stricter than the bare `atlas anchor add`, which leaves the ToC line for a human, so this write is immediately `atlas check --deep`-clean. `skill` appends a new recipe to the resolved `SKILL.md` plus its ToC entry (SCARS §SKILL-TOC-LOAD-BEARING — never optional). `bug` appends the next `BUG-N` under `BUGS.md`'s `## Open`; `faq` appends the next `FAQ-NNN` to the FAQ file (root `FAQ.md` or `docs/FAQ.md`). Both `bug` and `faq` die with the matching `atlas init --bugs`/`--faq` hint when their surface is absent — `atlas remember` never scaffolds one itself. SPEC §16 documents the session write-back protocol; the MCP `atlas_remember` tool and the hook's parting nudge are deferred follow-up work, not part of this core.
- **`atlas context` — the tool-free context bundle + provenance hash (RM-25, SPEC §15).** Every existing consumption path assumes something — a SessionStart hook, an MCP client, a runtime adapter. `atlas context [--task "<t>"] [--budget N] [--format md|json] [--hash]` assumes nothing: it prints the complete orientation payload to stdout (ATLAS §0-1, then SCARS ToC, then SKILL ToC, then CLAUDE.md — in that priority order, each under its own `## ` heading), so ANY harness — a bare API call, a non-function-calling local model, a RAG pipeline, a human pasting into a chat — can prepend it. `--task` delegates to `atlas orient`'s relevance slice when `python3` is available, degrading to the full spine (with a stderr note) when it isn't — it never fails. `--budget N` caps the emitted bytes, dropping CLAUDE.md first, then the SKILL ToC, then the SCARS ToC (the ATLAS slice is never dropped), with a final hard truncate as a last resort so output NEVER exceeds N. `--hash` prints a stable `sha256` of exactly the emitted bundle plus a manifest (files, git commit, byte count) — **context provenance**: the same tree always hashes the same, so an agent run is citable ("ran against spine sha256:…"). `--format json` emits `{"bundle","hash","manifest"}`, `hash`/`manifest` always populated in that format. SPEC §15 declares the bundle THE canonical consumption form — the hook/MCP/adapters are conveniences over it, not separate sources of truth.

### Fixed

- **`atlas init --<surface-flag>` no longer re-scaffolds the whole quartet on an existing ATLAS repo (idea-ledger item, "Surface-only init mode").** Any optional-surface flag (`--loop`/`--bugs`/`--critics`/`--faq`/`--tools`/`--asop`/`--akigi`/`--frq`/`--brd`/`--srd`/`--intake`) previously ran the full quartet+seeds scaffold block *in addition to* the requested surface — on a repo that deliberately removed one of those files (e.g. a pruned `EXAMPLES.md`), this silently re-created it, twice (RM-46, and again 2026-07-26). Now: when at least one surface flag is passed **and** `ATLAS.md` already exists, `atlas init` skips the ATLAS/SKILL/SCARS/CLAUDE/EXAMPLES render, the `AGENTS.md` mirror, and the per-style seeds block entirely, scaffolding only the requested surface(s), and says so. A fresh directory (no `ATLAS.md` yet) and plain `atlas init` (no surface flags) are both unaffected — the documented one-shot fresh scaffold still produces the full conformant repo.
